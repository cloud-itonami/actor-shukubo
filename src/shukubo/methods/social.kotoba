(ns shukubo.methods.social
  "social.cljc — shukubo DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from shukubo's own
...
(defn draft-observation-post
  "A dry-run observation post over shukubo's own domain (Airbnb/Hotels-inverse lodging commons — Ring 0 hospitality-first (covenantal/pilgrim/mutual-aid stays, cash≡0 or cost-share) → Ring 1…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "shukubo R0: live social posting is Council Lv6+ + operator + "
